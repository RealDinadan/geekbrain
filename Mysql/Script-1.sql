...
DROP TABLE IF EXISTS Post; -- Таблица постов
CREATE TABLE Post(
	id SERIAL PRIMARY KEY,
    user_id BIGINT UNSIGNED NOT NULL,
    bodi TEXT,
    created_at DATETIME DEFAULT NOW(),
    target_media BIGINT unsigned, 
    
  FOREIGN KEY (user_id) REFERENCES users(id)
 , FOREIGN KEY (target_media) REFERENCES media(id)
   
    );
    
   DROP TABLE IF EXISTS likes_post;-- Таблица лайков постов
